...
for z = zrange
    ...
    temp_M = zeros(13,13);
    for channel = 0:1:2
        d_matrix(:,:,1,channel+1) = ( 1+m*sin(2*kn*x0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) )*testf;
        d_matrix(:,:,3,channel+1) = Nx*testf*sin(2*kn*x0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase);
        d_matrix(:,:,5,channel+1) = -Nx*testx/x_pixelsize *( 1+m*sin(2*kn*x0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) )+Nx*testf*m*2*kn*cos(thetaz)*cos( 2*kn*x0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase );
        d_matrix(:,:,6,channel+1) = -Nx*testy/x_pixelsize *( 1+m*sin(2*kn*x0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) );
        d_matrix(:,:,7,channel+1) = Nx*testz/z_pixelsize *( 1+m*sin(2*kn*x0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) )+Nx*testf*m*2*kn*sin(thetaz)*cos( 2*kn*x0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase );
    end
    for channel = 0:1:2
        d_matrix(:,:,2,channel+4) = ( 1+m*sin(2*kn*y0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) )*testf;
        d_matrix(:,:,4,channel+4) = Ny*testf*sin(2*kn*y0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase);
        d_matrix(:,:,5,channel+4) = -Ny*testx/x_pixelsize *( 1+m*sin(2*kn*y0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) );
        d_matrix(:,:,6,channel+4) = -Ny*testy/x_pixelsize *( 1+m*sin(2*kn*y0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) )+Ny*testf*m*2*kn*cos(thetaz)*cos( 2*kn*y0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase );
        d_matrix(:,:,7,channel+4) = Ny*testz/z_pixelsize *( 1+m*sin(2*kn*y0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) )+Ny*testf*m*2*kn*sin(thetaz)*cos( 2*kn*y0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase );
    end
        ...
        for jjj = 1:1:iii
            for channel = 0:1:2
                temp_M(iii, jjj) = temp_M(iii, jjj) + sum(sum(d_matrix(:,:,iii,channel+1).*d_matrix(:,:,jjj,channel+1)./...
                    (bg+Nx*( 1+m*sin(2*kn*x0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) )*testf )));
                temp_M(iii, jjj) = temp_M(iii, jjj) + sum(sum(d_matrix(:,:,iii,channel+4).*d_matrix(:,:,jjj,channel+4)./...
                    (bg+Ny*( 1+m*sin(2*kn*y0*cos(thetaz)+2*kn*z0*sin(thetaz)+2*pi/3*(channel-1)+global_phase) )*testf )));
            end
        end
    end
